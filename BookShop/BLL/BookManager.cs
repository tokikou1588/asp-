...
//////-----------------------------------------------------
        ...
        /// 
        /// </summary>
        /// <param name="id">商品的编号（主键ID）</param>
        public void CreateHtmlPage(int id)
        {
            Model.Book bookModel=dal.GetModel(id);
            if (bookModel != null)
            {
                string filePath = HttpContext.Current.Request.MapPath("/Template/BookTemplate.html");//找到模板文件。
                //读取模板文件内容。
                ...
                string dir = "/StaticPage/" + bookModel.PublishDate.Year + "/" + bookModel.PublishDate.Month + "/" + bookModel.PublishDate.Day + "/";
