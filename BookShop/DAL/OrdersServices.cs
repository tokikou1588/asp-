...
namespace BookShop.DAL
{
	...
	public class OrdersServices
	{
        ...
        /// <returns></returns>
        public decimal CreateOrder(int userId, string address, string orderId)
        {
            SqlParameter[] pars = { 
                                  new SqlParameter("@OrderId",SqlDbType.NVarChar,50),
            ...
            pars[0].Value = orderId;
            pars[1].Value = userId;
            pars[2].Value = address;
            pars[3].Direction = ParameterDirection.Output;
            DbHelperSQL.RunProcedure("Pro_CreateOrder", pars);//执行存储过程
            return Convert.ToDecimal(pars[3].Value);//获取存储过程中的输出参数的值。
        }
